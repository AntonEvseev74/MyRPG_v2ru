...
public class ИгроваяПанель extends JPanel implements Runnable {
    ...
    public ОбъектПредмет списокОбъектовПредметов[] = new ОбъектПредмет[10];
    public ОбъектПерсонаж списокОбъектовПерсонажей[] = new ОбъектПерсонаж[10];
    ...
    public void настройкаИгры() {
        менеджерУстановкиОбъектов.установитьОбъекты();
        вклМузыка(0);
    }
    ...
    public void paintComponent(Graphics g) {
        ...
        // КАРТА
        менеджерПлиток.рисовать(g2);

        // ОБЪЕКТЫ ПРДМЕТЫ
        for (int i = 0; i < списокОбъектовПредметов.length; i++) {
            if (списокОбъектовПредметов[i] != null){
                списокОбъектовПредметов[i].рисовать(g2, this);
            }
        }

        // ОБЪЕКТЫ ПЕРСОНАЖИ
        for (int i = 0; i < списокОбъектовПерсонажей.length; i++) {
            if (списокОбъектовПерсонажей[i] != null){
                списокОбъектовПерсонажей[i].рисовать(g2, this);
            }
        }

        // ИГРОК
        игрок.рисовать(g2); // PLAYER
